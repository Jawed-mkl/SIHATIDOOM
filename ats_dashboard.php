...
// Récupérer les informations de l'ATS connecté
$email = $_SESSION['email'];
$sql = "SELECT * FROM professionnels WHERE email='$email' AND usertype='ats'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) == 1) {
    $ats = mysqli_fetch_assoc($result);
    $ats_id = $ats['id'];
}

// Récupérer le nombre total de patients associés à l'ATS
$sql_total_patients = "SELECT COUNT(DISTINCT patients_id) AS total_patients FROM rendez_vous WHERE professionnels_id = '$ats_id'";
...
// Récupérer le nombre de rendez-vous en attente pour l'ATS
$sql_pending_appointments = "SELECT COUNT(*) AS pending_appointments FROM rendez_vous WHERE professionnels_id = '$ats_id' AND statut = 'En attente'";
...
$sql_sex_distribution = "SELECT sex, COUNT(*) AS count FROM patients 
                         WHERE id IN (SELECT DISTINCT patients_id FROM rendez_vous WHERE professionnels_id = '$ats_id')
                         GROUP BY sex";
...
<head>
    ...
    <title>Tableau de Bord ATS</title>
