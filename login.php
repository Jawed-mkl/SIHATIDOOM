    ...
    foreach ($tables as $table) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if ($table == 'professionnels') {
                ...
                $update_status_sql = "UPDATE professionnels SET status='en ligne' WHERE email='$email'";
                ...
                if ($user_type == 'patients') {
                    $update_status_sql = "UPDATE patients SET status='en ligne' WHERE email='$email'";
                    mysqli_query($conn, $update_status_sql);
                }
            }
            break;
    ...
    if ($user_type !== '') {
        switch ($user_type) {
            case 'admins':
                ...
                break;
            case 'patients':
                $redirect_url = 'patient_dashbord_new.php';
                break;
            case 'receptionists':
