...
// Récupérer les informations de l'ergothérapeute connecté
$email = $_SESSION['email'];
$sql = "SELECT * FROM professionnels WHERE email='$email' AND usertype='ergotherapeute'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) == 1) {
    $ergotherapeute = mysqli_fetch_assoc($result);
    $ergotherapeute_id = $ergotherapeute['id'];
}

// Récupérer le nombre total de patients associés à l'ergothérapeute
$sql_total_patients = "SELECT COUNT(DISTINCT patients_id) AS total_patients FROM rendez_vous WHERE professionnels_id = '$ergotherapeute_id'";
...
// Récupérer le nombre de rendez-vous en attente pour l'ergothérapeute
$sql_pending_appointments = "SELECT COUNT(*) AS pending_appointments FROM rendez_vous WHERE professionnels_id = '$ergotherapeute_id' AND statut = 'En attente'";
...
$sql_sex_distribution = "SELECT sex, COUNT(*) AS count FROM patients 
                         WHERE id IN (SELECT DISTINCT patients_id FROM rendez_vous WHERE professionnels_id = '$ergotherapeute_id')
                         GROUP BY sex";
...
<head>
    ...
    <title>Tableau de Bord Ergothérapeute</title>
