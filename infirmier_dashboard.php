...
$sql = "SELECT * FROM professionnels WHERE email='$email' AND usertype='infirmier'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) == 1) {
    $infirmier = mysqli_fetch_assoc($result);
    $infirmier_id = $infirmier['id'];
}

// Récupérer le nombre total de patients associés à l'infirmier
$sql_total_patients = "SELECT COUNT(DISTINCT patients_id) AS total_patients FROM rendez_vous WHERE professionnels_id = '$infirmier_id'";
...
// Récupérer le nombre de rendez-vous en attente pour l'infirmier
$sql_pending_appointments = "SELECT COUNT(*) AS pending_appointments FROM rendez_vous WHERE professionnels_id = '$infirmier_id' AND statut = 'En attente'";
...
$sql_sex_distribution = "SELECT sex, COUNT(*) AS count FROM patients 
                         WHERE id IN (SELECT DISTINCT patients_id FROM rendez_vous WHERE professionnels_id = '$infirmier_id')
                         GROUP BY sex";
